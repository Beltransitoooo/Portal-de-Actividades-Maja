...
export const LoginForm = ({ email, setEmail, password, setPassword, onSubmit, onToggle, loading, error }) => {
    ...
    // 1. Configuramos el hook de Google
    const handleGoogleLogin = useGoogleLogin({
        onSuccess: async (tokenResponse) => {
            // Mandamos el token al backend a través de nuestro hook
            const success = await loginWithGoogle(tokenResponse.access_token);
            if (success) {
                // Si todo sale bien, lo mandamos a la selección de módulos
                navigate('/dashboard'); 
            }
        },
        ...
        <div className="w-full max-w-[400px] bg-gradient-to-b from-white to-slate-50 rounded-[40px] p-8 lg:p-10 border-[5px] border-white shadow-[0_30px_30px_-20px_rgba(11,19,43,0.15)] mx-auto">
            ...
            <div className="mt-8">
                ...
                <div className="w-full flex justify-center gap-4 mt-4">
                    {/* BOTÓN DE GOOGLE CON FUNCIONALIDAD AGREGADA */}
                    <button 
                        type="button"
                        onClick={() => handleGoogleLogin()} 
                        className="bg-gradient-to-tr from-gray-800 to-gray-600 border-[4px] border-white p-2 rounded-full w-12 h-12 flex items-center justify-center shadow-md transition-all duration-200 hover:scale-110 active:scale-90"
                    >
                    ...
                    {/* Botón de Apple (Queda igual, visualmente intacto) */}
                    <button type="button" className="bg-gradient-to-tr from-gray-800 to-gray-600 border-[4px] border-white p-2 rounded-full w-12 h-12 flex items-center justify-center shadow-md transition-all duration-200 hover:scale-110 active:scale-90">
